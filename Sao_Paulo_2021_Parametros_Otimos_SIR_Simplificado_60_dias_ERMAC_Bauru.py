...
# Dados Reais 
dado_real = np.loadtxt('casos_acumulados.txt')

# Busca Parametros Otimos São Paulo 60 dias ERMAC Bauru

#Os melhores parametros encontrados de alpha, beta, I0 e intervalo de integração se encontra abaixo

#Taxas de Infeccao e Recuperação
alpha=0.665 
beta=0.501
...
#Comparação do gráfico dos casos oficiais de COVID-19 no Município de São Paulo com os dados simulados    
plt.figure()
plt.plot(dado_real)
...
alpha_vec = np.linspace(0.5, 0.95, 451)
beta_vec = np.linspace(0.5, 0.95, 451)
...
matriz_recortada= matriz_simulacao[0:101474,:]
...
"""
# Encontrando menor erro para I0=[1,10]

I0 = 0
# Erro para 60 dias
erro_vec=np.zeros([10,2])
for k in range(0,10):
    I0=I0+1
    condicao_inicial = [S0, I0, R0]
    t_vector, S_vector, I_vector, R_vector = rk4_sistemas3por3(S_dot, I_dot, R_dot, \
